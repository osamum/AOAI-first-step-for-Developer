...
9. **lm.js** のコードが正しく動作するか、実行して確認します
    ...
    * [**Azure OpenAI の言語モデルにメッセージを送信するサンプル**](samples/lm_0.js)
...
2. オウム返しアプリケーションの **consoleBot.js**  ファイルをオープンし、ファイルの先頭にあるコメント `//[PLACEHOLDER:require lm.js]` を以下のコードに置き換えます

    ```javascript
    cconst lm = require('./AOAI/lm.js');
    ```
...
4. **consoleBot.js** を実行して Azure OpenAI サービスの言語モデルと継続的に会話ができるか確認します
    ...
    コンソール画面に表示された `Prompt:` の後に任意の文字列を入力してキーボードの \[**Enter**\] キーを押下し、Azure OpenAI サービスの言語モデルからの応答が返ることを確認します

    会話が継続的に行えることを確認したらキーボードの \[**Ctrl**\] + \[**C**\] キーを押下してプログラムを終了します
