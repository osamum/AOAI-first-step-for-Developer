...
Responses API での **Structured Outputs** のリクエスト構造は変更になってるいるので注意が必要ですが、コーディングに関しては Chat Completions API とほぼ同じです。 zodResponseFormat が **zodTextFormat** に変わったくらいです。
...
async function sendMessage(message) {
    ...
    const response = await client.responses.parse({
        ...
        input: messages,
        text: {
            format: zodTextFormat(RecepiResponse,'RecepiResponse')
        }
    });
...
👈 [**演習 3. 6 : Web を検索して回答する**](Ex03-6.md)
