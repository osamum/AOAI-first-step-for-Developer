...
この演習で実施しているコードの変更は、lm.js ファイル内の **sendMessage** 関数と、 consoleBot.js ファイル内の **process.stdin.on** ハンドラーの二か所ですが、**consoleBot.mjs** に行う変更内容は、consoleBot.js ファイルへの変更内容と同じです。

よって、**lm.mjs ファイル** 内の **sendMessage** 関数を Responses API を使用した場合のコードに変更するサンプルコードを以下に示します。
...
async function sendMessage(inputText, imageUrls) {
    if (inputText) {
        if (imageUrls && imageUrls.length > 0) {
            const message = { role: "user", content: [{ type: "input_text", text: inputText }]};
            for (const url of imageUrls) {
                message.content.push({ type: "input_image", image_url: url });
            }
