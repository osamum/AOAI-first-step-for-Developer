...
Chat Completions API と Responses API では **tools** の構造と、関数の処理結果を言語モデルに送って回答を得るまでの流れが異なるので、注意が必要です。

また、ファイル funcs.js の名前も **funcs.mjs** に変更してください。

Responses API を使用した場合のサンプルコードを以下のようになります。なお、Responses API は[画像生成の機能](https://learn.microsoft.com/ja-jp/azure/ai-foundry/openai/how-to/responses?tabs=python-key#image-generation-preview)もサポートしていますが、現状(2025年10月時点)、正常に動作させることができなかったため、従来のとおりとしています。
...
import myFunctions from './funcs.mjs';
import imageGen from './imgGen.mjs';

const endpoint = process.env["AZURE_OPENAI_ENDPOINT"] ;
const apiKey = process.env["AZURE_OPENAI_API_KEY"] ;
const modelName = "gpt-4o-mini";

let messages = [{ role: "system", content: "You are an useful assistant."},];

//Azure OpenAI にメッセージを送信する関数
async function sendMessage(inputText, imageUrls) {
    if (inputText) {
    ...
    for (const item of response.output) {
        ...
        if (item.type == "function_call") {
            return await sendFunctionResult(item);
...
const tools = [
    ...
    },
    {
        type: "function",
        name: "get_GitHubUser_info",
        description: "GitHub アカウントの情報を返します",
        parameters: {
            type: "object",
            properties: {
                userName: {
...
//アプリケーション内で実行した関数の結果を言語モデルに返す
async function sendFunctionResult(item) {
    const args = JSON.parse(item.arguments);
    const functionResponse = await routingFunctions(item.name, args);
    messages.push({ type: "function_call_output", call_id: item.call_id, output: functionResponse });
    return await sendMessage();
}
...
```
また、Responses API は MCP の呼び出しもサポートしており、例えば、[Microsoft Learn の MCP](https://learn.microsoft.com/ja-jp/training/support/mcp) のように認証処理が不要な場合、以下の JSON を tools に追加することで使用可能になります。
...
```
認証が必要な MCP サーバーを使用する場合は、MCP トークンの取得と更新の処理をアプリケーションに実装する必要があります。詳細は以下のドキュメントをご参照ください。

* [**リモート MCP サーバーの使用**](https://learn.microsoft.com/ja-jp/azure/ai-foundry/openai/how-to/responses?tabs=python-key#using-remote-mcp-servers)
