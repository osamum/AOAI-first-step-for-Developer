...
> [!IMPORTANT]
> 画像生成のモデルが言語モデルと異なるリージョン、インスタンスにデプロイされている(※エンドポイントのサブドメインが 2 つのモデルで異なる)場合、以下の変更を行ってください。
> 1. `.env` ファイルに以下の設定を追加
>    ```env
>    IMAGEN_ENDPOINT=共有されている埋め込みモデルのエンドポイントのドメイン部分 (※例: https://dall-e-xyz..openai.azure.com) を記述
>    IMAGEN_API_KEY=共有されている埋め込みモデルの API キーを記述
>   ```
