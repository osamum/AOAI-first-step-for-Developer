...
> [!NOTE] この演習では言語モデルに関数を生成させますが、試用サブスクリプションの場合、コードを生成する際、Token 数が上限を超えてしまう可能性があります。その場合は、[Microsoft Copilot](https://copilot.microsoft.com/) を使用してコードを生成することができます。
