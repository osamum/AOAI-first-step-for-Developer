...
>現在、Azure OpenAI サービスでは、後継の [Responses API](https://learn.microsoft.com/ja-jp/azure/ai-foundry/openai/how-to/responses) が利用可能です。しかし、Azure AI Foundry や Ollama で **OpanAI 以外のモデル**をホストする場合に提供される OpenAI 互換 API は Chat Completion API をベースにしているため、ここでは Chat Completion API を使用します。なお、この演習で作成するボットアプリケーションは、エンドポイントと API キーを、Azure AI Foundry でホストされる DeepSeek などに置き換えることで、同様に動作させることができます。(※ただし、画像認識など、モデルが対応していない機能は利用できません) もし、Responses API を使用したい場合は、ボットアプリケーション完成後に OpenAI が公開している[Migrate to the Responses API](https://platform.openai.com/docs/guides/migrate-to-responses)を参考にコードを書き換えてみてください。
