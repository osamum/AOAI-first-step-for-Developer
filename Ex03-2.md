...
>[!NOTE]
>この演習用ボットアプリケーションは [Chat Completion API](https://learn.microsoft.com/ja-jp/azure/ai-foundry/openai/how-to/chatgpt) を使用して作成します。
>現在、Azure OpenAI サービスでは、後継の [Responses API](https://learn.microsoft.com/ja-jp/azure/ai-foundry/openai/how-to/responses) が利用可能です。しかし、Azure AI Foundry や Ollama で **OpanAI 以外のモデル**をホストする場合に提供される OpenAI 互換 API は Chat Completion API をベースにしているため、ここでは Chat Completion API を使用します。なお、この演習で作成するボットアプリケーションは、エンドポイントと API キーを、Azure AI Foundry でホストされる DeepSeek、gpt-oss、Phi4 などに置き換えることで、同様に動作させることができます。(※ただし、画像認識など、モデルが対応していない機能は利用できません。gpt-oss、Phi4 モデルは Function Calling をサポートしないので使用する際はリクエストに `tools` を含めないでください) 
>もし、Responses API を使用したい場合は、各タスクの最後に Responses API を使用した場合のサンプルコードを掲載していますので参考にしてください。また Chat Completion API から Responses API については OpenAI が公開している[Migrate to the Responses API](https://platform.openai.com/docs/guides/migrate-to-responses)を参考にコードを書き換えてみてください。
...
# 参考 :  Responses API を使用した場合のサンプルコード

Responses API を使用するには、**.env** ファイルに記述されているエンドポイントの最後に、以下のように **/v1** を[追加します](https://learn.microsoft.com/ja-jp/azure/ai-foundry/openai/api-version-lifecycle?tabs=python#api-evolution)。

`https://your-resource-name.openai.azure.com/openai/v1`

また、環境変数名を以下のように変更すると、

```
OPENAI_BASE_URL=ここにエンドポイントを記述
OPENAI_API_KEY=ここに API キーを入力を記述
```

client オブジェクトのインスタンス生成時に自動的に環境変数の内容が読み取られ、以下のように引数の指定が不要になります。

```javascript
const client = new AzureOpenAI();
```
もちろん、従来のように以下のように引数で明示的に指定しても動作します。

```javascript
const client = new AzureOpenAI({baseURL:endpoint, apiKey:apiKey});
```

コードの記述については、AI 関連の開発で使用する Node.js 用のライブリで [ECMA Script](https://nodejs.org/api/esm.html) 形式のモジュールしかサポートされていないものがあるので、モジュールの読み込みには import 文を使用し、export 文で関数を公開する形式で記述します。

また、ファイルの拡張子を **.mjs** とします。

よって、consoleBot.js は **consoleBot.mjs** となり、記述するコードは以下のようになります。


```javascript
import lm from './AOAI/lm.mjs';
...
同様に、lm.js は **lm.mjs** となり、記述するコードは以下のようになります。
...
//Azure OpenAI にメッセージを送信する関数
async function sendMessage(message) {
    //環境変数がOPENAI_BASE_URL,OPENAI_API_KEYの場合は引数不要
    //const client = new AzureOpenAI(); 
    const client = new AzureOpenAI({baseURL:endpoint, apiKey:apiKey});
